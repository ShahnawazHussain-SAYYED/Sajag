...
NEWS_TTL = 180  # 3 minutes: near real-time, and repeated taps do not hammer the news sites
...
def google_news(kind: str, term: str, area: Optional[str], iso: Optional[str]) -> list[dict]:
...
def gather(req: NewsReq) -> list[dict]:
    area = (req.area or "").strip() or None
    found = google_news(req.kind, req.term, area, req.iso)
    if len({a["url"] for a in found}) < 5:
        try:
            found += bing_news(req.kind, req.term, area)
        except Exception:
            pass
    if len({a["url"] for a in found}) < 5:
    ...
    merged: dict = {}
    for a in found:
        if a["ts"] and now - a["ts"] > MAX_AGE_MS:
            continue
        k = a["url"]
        if k in merged:
...
def news(req: NewsReq, request: Request):
    ...
    payload = {"kind": req.kind, "loc": req.loc, "area": req.area, "fetchedAt": int(time.time() * 1000),
               "scanned": len(arts), "ai": ai_ok, "items": items[:40]}
...
def news_debug(request: Request, term: str = "Mumbai", kind: str = "crime", iso: str = "IN"):
    ...
    out: dict = {}
    for name, fn in (
        ("google_16h", lambda: google_raw(f"{quote(term)} ({WORDS[kind]})", "16h", iso)),
        ("google_30d", lambda: google_raw(f"{quote(term)} ({WORDS[kind]})", "30d", iso)),
