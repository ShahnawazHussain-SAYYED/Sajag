...
VERSION = "news-v5-wide"  # shown in /health and in the app, so you can confirm the new backend is live
...
# ---- wider net: local-language news, Google's own local feed, broad area searches ----
LOCAL_EDITIONS = {"hi": ("hi", "IN", "IN:hi"), "mr": ("mr", "IN", "IN:mr")}
LOCAL_WORDS = {
    "crime": {"hi": "पुलिस OR चोरी OR हत्या OR गिरफ्तार OR लूट OR हमला OR ठगी OR चाकू OR मारपीट",
              "mr": "पोलीस OR चोरी OR हत्या OR अटक OR लूट OR हल्ला OR फसवणूक OR चाकू OR मारहाण"},
    "scam": {"hi": "साइबर OR ठगी OR धोखाधड़ी OR \"डिजिटल अरेस्ट\" OR ऑनलाइन फ्रॉड",
             "mr": "सायबर OR फसवणूक OR ठगी OR \"डिजिटल अरेस्ट\" OR ऑनलाइन फसवणूक"},
}
_names: dict = {}   # (city, area) -> {"hi": "...", "mr": "..."}; one AI call per place, remembered


def local_names(term: str, area: Optional[str], iso: Optional[str]) -> dict:
    """The place name as Hindi and Marathi newspapers write it. Most small local news is in these languages."""
    if (iso or "").upper() != "IN":
        return {}
    key = (slug(term), slug(area or ""))
    if key in _names:
        return _names[key]
    place = f"{area + ', ' if area else ''}{term}, India"
    try:
        d = ask(f'Write the place "{place}" the way Hindi and Marathi newspapers write its name, in Devanagari script. '
                'Return ONLY JSON {"hi":"","mr":""} with just the place name (no extra words).', 300)
    except AIError:
        return {}
    out = {k: str(d.get(k) or "").strip()[:40] for k in ("hi", "mr") if re.search(r"[\u0900-\u097F]", str(d.get(k) or ""))}
    if len(_names) > 2000:
        _names.clear()
    _names[key] = out
    return out
...
def _items(root: ET.Element, source_default: str = "") -> list[dict]:
    out = []
    for it in root.iter("item"):
        title, link = it.findtext("title") or "", it.findtext("link") or ""
        if not title or not link:
            continue
        src = it.find("source")
        source = (src.text or "").strip() if src is not None else source_default
        try:
            ts = int(parsedate_to_datetime(it.findtext("pubDate") or "").timestamp() * 1000)
        except Exception:
            ts = 0
...
def google_local(kind: str, term: str, area: Optional[str], iso: Optional[str]) -> list[dict]:
    """Hindi and Marathi Google News editions, searched with the place name in Devanagari."""
    out, errs = [], []
    for lang, name in local_names(term, area, iso).items():
        hl, gl, ceid = LOCAL_EDITIONS[lang]
        for w in ("1d", "7d"):
            try:
                root = fetch_rss("https://news.google.com/rss/search", {"q": f'"{name}" ({LOCAL_WORDS[kind][lang]}) when:{w}', "hl": hl, "gl": gl, "ceid": ceid})
                out += [{**a, "local": True} for a in _items(root)]
            except Exception as e:
...
def google_geo(term: str, iso: Optional[str]) -> list[dict]:
    """Google News' own 'local' feed for a place: every latest headline there, not only crime words."""
    hl, gl, ceid = EDITIONS.get((iso or "").upper(), EDITIONS["US"])
    root = fetch_rss("https://news.google.com/rss/headlines/section/geo/" + urlquote(term.strip()), {"hl": hl, "gl": gl, "ceid": ceid})
    return [{**a, "local": False} for a in _items(root)]


def google_broad(term: str, area: Optional[str], iso: Optional[str]) -> list[dict]:
    """The place name alone, newest windows only. Crime relevance is judged afterwards from the headline."""
    q = quote(area) if area and area.strip().lower() != term.strip().lower() else quote(term)
    out, errs = [], []
    for w in ("2h", "1d"):
        try:
            out += [{**a, "local": bool(area)} for a in google_raw(q, w, iso)]
        except Exception as e:
...
def run_sources(jobs: list, diag: dict) -> list[dict]:
    """Ask every source at the same time. A slow or failing source never blocks the others."""
    out: list = []
    ex = ThreadPoolExecutor(max_workers=8)
    futs = {ex.submit(fn): name for name, fn in jobs}
    try:
        for f in as_completed(futs, timeout=26):
            name = futs[f]
            try:
                r = f.result()
                out += r
                diag[name] = {"items": len(r), "error": None}
            except Exception as e:
                diag[name] = {"items": 0, "error": f"{type(e).__name__}: {str(e)[:70]}"}
    except Exception:
        pass
    for f, name in futs.items():
        if name not in diag:
            diag[name] = {"items": 0, "error": "timeout"}
...
LOCAL_RULES = [("theft", r"चोरी"), ("robbery", r"लूट"), ("assault", r"हत्या|हल्ला|हमला|चाकू|मारपीट|मारहाण"),
               ("fraud", r"ठगी|फसवणूक|धोखाधड़ी|साइबर|सायबर"), ("other", r"पुलिस|पोलीस|गिरफ्तार|अटक")]
...
def heuristic(kind: str, a: dict) -> Optional[dict]:
    ...
    for typ, pat in LOCAL_RULES:            # Hindi / Marathi headlines
        if re.search(pat, a["title"]):
            return {"type": typ if kind == "crime" else "other", "place": None, "in_area": True, "time_of_day": "unknown"}
    return None
...
def extract(req: NewsReq, arts: list[dict]) -> tuple[dict, bool]:
    ...
    prompt = (
        ...
        'Return ONLY JSON: {"items":[{"i":0,"type":"","place":"neighbourhood, road, station or locality as named in the text, or null","in_area":true,"time_of_day":"day|evening|night|unknown","en":"short English translation if the headline is not in English, else null"}]}'
    )
    try:
        d = ask(prompt, 4500)
    except AIError:
...
def gather(req: NewsReq) -> tuple[list[dict], dict]:
    ...
    diag: dict = {}
    jobs = [
        ("google", lambda: google_news(req.kind, req.term, area, req.iso, {})),
        ("google_broad", lambda: google_broad(req.term, area, req.iso)),
        ("google_local_feed", lambda: google_geo(req.term, req.iso)),
        ("hindi_marathi", lambda: google_local(req.kind, req.term, area, req.iso)),
        ("bing", lambda: bing_news(req.kind, req.term, area)),
        ("gdelt", lambda: gdelt(req.kind, req.term)),
    ]
    found = run_sources(jobs, diag)
    now = time.time() * 1000
    merged: dict = {}
    for a in found:
...
def news(req: NewsReq, request: Request):
    ...
    key = f"v3_{req.kind}_{(req.iso or 'xx').lower()}_{slug(req.area or '')}_{slug(req.term)}"
    if not req.force:
    ...
    if arts:
        ...
        for a in arts:
            ...
            items.append({"title": a["title"], "url": a["url"], "source": a["source"], "ts": a["ts"], "date": iso_date(a["ts"]),
                          ...
                          "titleEn": (str(x["en"])[:200] if x.get("en") else None)})
    payload = {"kind": req.kind, "loc": req.loc, "area": req.area, "fetchedAt": int(time.time() * 1000),
               "scanned": len(arts), "ai": ai_ok, "ver": VERSION, "src": diag, "items": items[:60]}
